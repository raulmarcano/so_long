...
/*   Updated: 2024/06/26 12:45:57 by rmarcano         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_images(t_game *game, t_map *map, t_sprite *sprites)
{
	...
	while(map->carte[y])
	{
		...
		{
			if(map->carte[y][x] == '1')
				mlx_put_image_to_window(game->mlx, game->window, sprites->wall, x * 64, y * 64);
			else if(map->carte[y][x] == '0')
				mlx_put_image_to_window(game->mlx, game->window, sprites->floor, x * 64, y * 64);
			else if(map->carte[y][x] == 'P')
				mlx_put_image_to_window(game->mlx, game->window, sprites->player, x * 64, y * 64);
			else if(map->carte[y][x] == 'C')
				mlx_put_image_to_window(game->mlx, game->window, sprites->coin, x * 64, y * 64);
			else if(map->carte[y][x] == 'E')
				mlx_put_image_to_window(game->mlx, game->window, sprites->exit, x * 64, y * 64);
			x++;
		}
		y++;
	}
